...
class TestConsumerGetQueues:
    def test_get_queues_strict(self):
        consumer = Consumer(
            client=Client(role="consumer"), priority="strict", queues=["a", "b", "c"]
        )
    ...
    def test_get_queues_uniform(self):
        consumer = Consumer(
            client=Client(role="consumer"), priority="uniform", queues=["a", "b", "c"]
        )

        count_table = xr.DataArray(
            np.zeros((3, 3)),
            dims=("queue", "count"),
            coords={"queue": ["a", "b", "c"], "count": [0, 1, 2]},
        )
